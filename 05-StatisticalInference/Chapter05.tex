...
{Statistical Hypotheses}
{Example}
Suppose you own a company that sells green peas\footnote[1]{\tiny We could use any other item from your grocery list, but why not pay a little tribute to Gregor Mendel?}, and that you want to determine whether your $500$g packages really contain their nominal weight (at least on average).
\vhalf
...
\vone
If $H_0$ is true (i.e., if $\mu=500$g), the distribution of values of $\bar{x}$ is aproximately normal (assuming the CLT holds), with average $500$ and variance given by $s^2/n$;
\vone
For a desired Type-I error probability $\alpha=0.05$, the critical values of the distribution of $\bar{x}$ are the ones for which the probability content within the acceptance region is $1-\alpha = 0.95$.
\begin{tikzpicture}[remember picture,overlay]
...
\vone
This result implies a probability of $\left(1-\alpha\right)$ that $Z_0$ will fall within the range $(\pm z_{1-\alpha/2})$\footnote[2]{\tiny $z_{1-\alpha/2}$ is the superior $100(1-\alpha/2)\%$ percentile of the standard normal distribution;} if $H_0$ is true, which provides a selection criterion between $H_0$ and $H_1$:

\bitems If $|z_0|>z_{1-\alpha/2}$, we reject $H_0$ at the confidence level $1-\alpha$;
...
\vone
It can be shown that, \textbf{if $H_0$ is true}, then
...
\vhalf
The critical value of this test statistic for the desired significance is $t_{\alpha,n-1} = t_{0.01,9} = -2.82$ --- i.e., under $H_0$ there is a $99\%$ change that the test statistic will yield a value greater than this number.
\vone
...
{Some considerations}
The probability of Type-II error can be easily (and wrongly) evaluated \textit{a posteriori}, but its definition \textit{a priori} requires some care;
\vone
...
\vone
The same sample of $n=10$ packs is used. Assume that you estimated a reasonable standard deviation for this sample as $s=10g$. From this data, we can compute the power of this test as:
\begin{columns}
