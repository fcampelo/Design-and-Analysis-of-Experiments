...
\author[]{Felipe Campelo\\{\footnotesize f.campelo@aston.ac.uk}}
\institute{Computer Science Group}
\date{\scriptsize Birmingham\\March 2019}
...
\begin{frame}
\begin{flushright}
\includegraphics[width=.25\textwidth]{../figs/Aston_375x134}
\end{flushright}
  \titlepage
  \begin{tikzpicture}[remember picture,overlay]
  \node[anchor=south east,xshift=-5pt,yshift=122pt] at (current page.south east) {\tiny Version 2.2.2019};
...
{Statistical Hypotheses}
{Example}
Suppose that you have implemented an algorithm and you want to verify whether your implementation is correct. 
A theoretical model of the algorithm behaviour indicates that its convergence should happen on average at iteration $50$,
so a good initial test would be to run your algorithm a few times and check if it conforms with this prediction.
\vhalf
In this case the null hypothesis could be defined as:
\textit{the average iteration at which convergence is achieved is $50$}, and the alternative of interest could be expressed as the complementary inequality.
\beqs\begin{cases}
H_0: \mu = 50 \\
H_1: \mu \neq 50
\end{cases}\eqs
...
\bitems If $\bar{x} \cong 50$ - corroboration of $H_0$;
\item If $\bar{x} \ll 50$ or $\bar{x} \gg 50$ - refutation of $H_0$;
\eitem
\vone
...
\vhalf
The critical value of this test statistic for the desired significance is $t_{\alpha}^{(n-1)} = t_{0.99}^{(9)} = 2.82$, which means that under $H_0$ there is a $99\%$ change that the test statistic will yield a value smaller than this threshold.
\vone
...
\item A. Reinhart, \textit{Statistics Done Wrong: the woefully complete guide}. No Starch Press, 2015 ({\tiny\url{http://www.statisticsdonewrong.com}})
