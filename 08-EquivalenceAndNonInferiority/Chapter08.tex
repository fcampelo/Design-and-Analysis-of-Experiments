...
\vone
Another useful class of experiments in engineering and science is one in which the experimenter is interesting in investigating \textit{equivalence} (within a given error margin), for instance:
...
{Testing equivalence}
{Introduction}
In equivalence testing, the situation is reversed: the (approximate) equality of two parameters is the novelty one hopes to establish. 
\vone
Consequently, the burden of proof shifts to providing evidence that there is no difference.
\vone
...
\vone
In the case of non-inferiority tests, one can in principle use a regular test of differences with a one-sided alternative (which would be equivalent to setting $\delta^* = 0$).
\end{ftst}
...
A simple way of thinking about testing equivalence consists in thinking about confidence intervals instead of p-values:

\begin{colorblock}{}{bg=green!30,fg=black}
\centering ``\textit{Equivalence can be established}''
\end{colorblock}
